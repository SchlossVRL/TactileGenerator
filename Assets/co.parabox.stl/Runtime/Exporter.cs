						...
						using (BinaryWriter writer = new BinaryWriter(File.Open(path, FileMode.Create), new ASCIIEncoding()))
						{
							...
							foreach(Mesh mesh in meshes)
							{
								...
								for(int i = 0; i < triangleCount; i += 3)
								{
									...
									writer.Write(v[a].x * 1000.0f);
									writer.Write(v[a].y * 1000.0f);
									writer.Write(v[a].z * 1000.0f);

									writer.Write(v[b].x * 1000.0f);
									writer.Write(v[b].y * 1000.0f);
									writer.Write(v[b].z * 1000.0f);

									writer.Write(v[c].x * 1000.0f);
									writer.Write(v[c].y * 1000.0f);
									writer.Write(v[c].z * 1000.0f);
		...
		public static string WriteString(IList<Mesh> meshes, bool convertToRightHandedCoordinates = true)
		{
			...
			foreach(Mesh mesh in meshes)
			{
				...
				for(int i = 0; i < triLen; i+=3)
				{
					...
					sb.AppendLine(string.Format("\tvertex {0} {1} {2}", v[a].x * 1000.0f, v[a].y * 1000.0f, v[a].z * 1000.0f));
					sb.AppendLine(string.Format("\tvertex {0} {1} {2}", v[b].x * 1000.0f, v[b].y * 1000.0f, v[b].z * 1000.0f));
					sb.AppendLine(string.Format("\tvertex {0} {1} {2}", v[c].x * 1000.0f, v[c].y * 1000.0f, v[c].z * 1000.0f));
